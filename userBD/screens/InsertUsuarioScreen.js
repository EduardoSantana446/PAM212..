...
export default function InsertUsuarioScreen() {
  ...
  const handleGuardarEdicion = async () => {
    try {
      await controller.actualizarUsuario(usuarioAEditar, nuevoNombre);
  ...
  const renderUsuario = ({ item, index }) => (
    <View style={styles.userItem}>
      ...
      <View style={styles.userInfo}>
        ...
        <TouchableOpacity
          style={[styles.button, {backgroundColor:'#2600ffff'}]} 
          onPress={() => abrirModalEditar(item.id, item.nombre)}>
